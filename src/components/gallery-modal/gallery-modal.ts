...
export class GalleryModal implements OnInit {
  ...
  private displayIndex: boolean = false;
  ...
  constructor(private viewCtrl: ViewController, params: NavParams, private element: ElementRef, private platform: Platform, private domSanitizer: DomSanitizer, private commonProvider: CommonProvider) {
    this.photos = params.get('photos') || [];
    this.closeIcon = params.get('closeIcon') || 'arrow-back';
    this.displayIndex = params.get('displayIndex') || false;
   ...
   */
  public dismiss() {
    this.commonProvider.sendCurrentSlideInfo(this.currentSlide);
  ...
  private slideChanged(event) {
    this.currentSlide = this.slider.getActiveIndex() > this.photos.length - 1 ? this.photos.length - 1 : this.slider.getActiveIndex();
  }
